...
| `SIBYL_TOKENS_PER_USER_PER_MONTH` | unset | prompt plus completion tokens one platform user may spend per UTC calendar month, summed from the daily totals in `SIBYL_DB_PATH`. A bearer with the `admin` role has no monthly cap. Set it so the number of platform users times it, at the model prices, stays under `SIBYL_MONTHLY_SPEND_LIMIT_USD`. Needs `PLATFORM_JWT_SECRET` |
...
- `POST /sessions/{id}/messages` `{"content"}` appends a user message without running the model. Content over 32768 bytes is a 400 and is not stored
...
The rest of the context is trimmed for you. Only the newest 200 messages after the stored summary are read. Once a request is estimated over 100000 tokens or holds those 200 messages, the last 20 messages stay verbatim and everything older goes to the run's own model to be summarized into one line of the system prompt, which is stored on the session so it is not summarized again. A summarization that fails drops the oldest messages for that request only. A tool result over 20000 characters is cut there and ends with `[truncated]`, in the `tool_return` event as well as in the history.
