...
| `SIBYL_TOKENS_PER_USER_PER_MONTH` | unset | prompt plus completion tokens one platform user, `admin` included, may spend per UTC calendar month, summed from the daily totals in `SIBYL_DB_PATH`. Set it so the number of platform users times it, at the model prices, stays under `SIBYL_MONTHLY_SPEND_LIMIT_USD`. Needs `PLATFORM_JWT_SECRET` |
